...
def decode_onehot(labels, mask):
    ''' convert from one-hot encoded to numeric representation'''
    cat_labels = np.zeros(labels.shape[0])
    cat_labels[mask] = np.argmax(labels[mask], 1) + 1
    return cat_labels.astype(int)
